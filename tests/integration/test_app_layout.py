...
def app():
    ...
    blueprint.add_url_rule("/classes", "index", lambda: render_template_string(PAGE, note="x"))
...
def test_current_section_is_marked_for_assistive_tech(app):
    ...
    assert 'href="/classes" title="Classes"' in html
...
def test_layout_scripts_and_styles_are_served(app):
    ...
    html = client.get("/classes").get_data(as_text=True)

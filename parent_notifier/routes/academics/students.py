...
from parent_notifier.forms.student_results import rows_for, rows_from_form
...
def _figures(class_group, semester, student=None, posted=True):
    """The subject rows: as posted, or as saved when the page first opens."""
    if posted and request.method == "POST":
        return rows_from_form(request.form, semester, class_group.midsem_max)
    saved = students.results_of(semester, student) if student else {}
    return rows_for(semester, class_group.midsem_max, saved)


def _cells(rows) -> dict:
    return {row.subject_id: row.cell() for row in rows}


def _form_page(class_group, semester, form, rows, student=None, delete_form=None):
    ...
    return render_template(
        ...
        form=form,
        rows=rows,
        figure_errors=[row.error for row in rows if row.error],
        student=student,
...
def add(class_id: int, number: int):
    ...
    rows = _figures(class_group, semester)
    # Both are checked, so every problem shows at once.
    valid = form.validate_on_submit()
    if valid and not any(row.error for row in rows):
        try:
            student = students.add_student(
                class_group, semester, form.enrollment_no.data, form.details(), _cells(rows)
            )
    ...
    return _form_page(class_group, semester, form, rows)
...
def edit(class_id: int, number: int, student_id: int):
    ...
    rows = _figures(class_group, semester, student)
    valid = form.validate_on_submit()
    if valid and not any(row.error for row in rows):
        students.update_student(student, form.details(), semester, _cells(rows))
        ...
        return redirect(url_for("semesters.workspace", class_id=class_id, number=number))
    return _form_page(class_group, semester, form, rows, student)
...
def delete(class_id: int, number: int, student_id: int):
    ...
    rows = _figures(class_group, semester, student, posted=False)  # the delete form was posted
    return _form_page(class_group, semester, edit_form, rows, student, delete_form=form)
